...
def decode(p:ImagePayload)->np.ndarray:
    try:b=base64.b64decode(p.data.split(',',1)[-1],validate=False)
...
def rectify(im:np.ndarray)->np.ndarray:
    ...
    contours,_=cv2.findContours(edges,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE);area_img=work.shape[0]*work.shape[1];best=None;best_score=-1.;cx,cy=work.shape[1]/2,work.shape[0]/2
    for c in sorted(contours,key=cv2.contourArea,reverse=True)[:100]:
        area=cv2.contourArea(c)
        if area<area_img*.018 or area>area_img*.97:continue
        poly=cv2.approxPolyDP(c,.021*cv2.arcLength(c,True),True)
        if len(poly)!=4 or not cv2.isContourConvex(poly):continue
        q=order_quad(poly);cw=max(np.linalg.norm(q[1]-q[0]),np.linalg.norm(q[2]-q[3]));ch=max(np.linalg.norm(q[3]-q[0]),np.linalg.norm(q[2]-q[1]))
        if min(cw,ch)<45:continue
        ratio=min(cw,ch)/max(cw,ch);ratio_score=max(0.,1-abs(ratio-.714)/.22);center_dist=np.linalg.norm(q.mean(0)-np.array([cx,cy]))/max(work.shape[:2]);center_score=max(0.,1-center_dist*1.8);rect=cv2.minAreaRect(c);box_area=max(1.,rect[1][0]*rect[1][1]);fill=min(1.,area/box_area);score=(area/area_img)*1.8+ratio_score*1.25+center_score*.45+fill*.35
...
def embed_one(card:np.ndarray)->np.ndarray:
    rgb=cv2.cvtColor(card,cv2.COLOR_BGR2RGB);rgb=cv2.resize(rgb,(224,224),interpolation=cv2.INTER_AREA).astype(np.float32)/255.;mean=np.array([.485,.456,.406],np.float32);std=np.array([.229,.224,.225],np.float32);x=((rgb-mean)/std).transpose(2,0,1)[None]
...
def feature_geometry(a:np.ndarray,b:np.ndarray)->tuple[float,float,int]:
    ...
    src=np.float32([ka[m.queryIdx].pt for m in good]).reshape(-1,1,2);dst=np.float32([kb[m.trainIdx].pt for m in good]).reshape(-1,1,2);_,mask=cv2.findHomography(src,dst,cv2.RANSAC,4.0);return orb_score,float(mask.mean()) if mask is not None else 0.,len(good)

def download_image(url:str)->np.ndarray:
    r=HTTP.get(url,timeout=(4,14));r.raise_for_status();im=cv2.imdecode(np.frombuffer(r.content,np.uint8),cv2.IMREAD_COLOR)
...
def rpc_match_signature(sig:str,count:int,category:str|None)->list[dict[str,Any]]:
    r=HTTP.post(f'{SUPABASE_URL}/rest/v1/rpc/cd_match_dinov2_signatures',headers=headers(),json={'query_signature':sig,'match_count':count,'category_filter':category},timeout=(4,14));r.raise_for_status();return r.json()
...
def fingerprint_url(req:FingerprintUrlRequest):
    try:
        card=download_image(req.url);v=robust_embedding(card);p,d=hashes(card);return {'signature':signature(v),'phash':p,'dhash':d,'model':MODEL_ID,'dimension':384,'time':now_ms()}
...
def verify(req:VerifyRequest):
    ...
    if not url:return {'verified':False,'warning':'reference image missing','card':public_card(row,0),'decision':{'state':'abstain','failClosed':True}}
